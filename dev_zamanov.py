...
def main():
    ...
    rotangle = rem_angle
    Rx_clock = np.array(
        [[1, 0, 0], [0, math.cos(rotangle), math.sin(rotangle)], [0, -math.sin(rotangle), math.cos(rotangle)]])
    b2 = Rx_clock.dot(b1)
    ...
    while tr - t0 <= 0.01:
        ...
        t1 = np.array([[0], [-r_p * math.cos(((2 * math.pi / 3) - theta_p) / 2)],
                       [-r_p * math.sin(((2 * math.pi / 3) - theta_p) / 2)]])
        ...
        print(Top_matrix)
        rangle_vectclock = np.array(
            [5 * math.pi / 6, -5 * math.pi / 6, math.pi / 6, math.pi / 2, -math.pi / 2, -math.pi / 6])
        # print("Rangle_val: ",rangle_vectclock)
        th = []
        for i in range(0, 6):
            Rx_clock = np.array([[1, 0, 0], [0, math.cos(rangle_vectclock[i]), math.sin(rangle_vectclock[i])],
                                 [0, -math.sin(rangle_vectclock[i]), math.cos(rangle_vectclock[i])]])
        ...
        # dxl_io.set_goal_position({1: (th[0] + zero_corr[0]), 2: (-th[1] + zero_corr[1]), 3: (th[2] + zero_corr[2]),
        #                           4: (-th[3] + zero_corr[3]), 5: (th[4] + zero_corr[4]), 6: (-th[5] + zero_corr[5])})
        if (firstflag == 0):
            time.sleep(7)
            firstflag = 1
        time.sleep(0.01)
